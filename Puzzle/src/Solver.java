	...
	public Solver(Board initial) {
		...
		while (!pqMain.isEmpty() && !(minMain = pqMain.delMin()).board.isGoal()) {
			...
			for (Board neighbor : minTwin.board.neighbors()) {
				if (minTwin.predecessor != null && minTwin.predecessor.board.equals(neighbor)) {
					continue;
				}
				final SearchNode searchNode = new SearchNode(minTwin.board, minTwin.predecessor, minTwin.moves);
				pqTwin.insert(new SearchNode(neighbor, searchNode, minTwin.moves + 1));
			}

			// process main
			for (Board neighbor : minMain.board.neighbors()) {
				if (minMain.predecessor != null && minMain.predecessor.board.equals(neighbor)) {
					continue;
				}
				final SearchNode searchNode = new SearchNode(minMain.board, minMain.predecessor, minMain.moves);
				pqMain.insert(new SearchNode(neighbor, searchNode, minMain.moves + 1));
			}
